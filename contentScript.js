...
function injectModal() {
  ...
  <div id="modalContent">
    <header id="modalHeader">
      <img src="${chrome.runtime.getURL("images")}/password-manager.png"><h3>SAVE LOGIN DETAILS FOR ${company_name}?</h3>
        ...
        <div class="passwordContainer">
          ...
          <button type="button" id="togglePassword">
            <span class="eye-icon-container">
              ...
              <i class="eye-slash-icon far fa-eye-slash" style="display: none;"></i>
            </span>
          </button>
        </div>
      </div>
...
function showModal(email, password) {
  ...
  document.getElementById('saveDetails').onclick = function() {
    ...
    const company_icon = window.location.href.slice(0, window.location.href.indexOf('.com/') + 4) + document.querySelector('.css-1t8sqvc').getAttribute('src');
    const companyName = window.location.href.slice(8, window.location.href.indexOf('.wd')).toUpperCase();
...
window.addEventListener('load', () => {
  ...
  font_awesome.href = 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css';

  document.head.appendChild(font_awesome);
  


  //console.log(chrome.runtime.getURL('images'));
